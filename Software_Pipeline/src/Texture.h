...
public:
	static  GLuint load2DTexture(const char* filename, GLint internalFormat = GL_RGBA,
		GLenum picFormat = GL_RGBA, int loadChannels = GL_RGB)
	{
		...
		unsigned char* data = stbi_load(filename, &picWidth, &picHeight, &nrComponents, 0);
		...
		if (data)
		{
			...
			glBindTexture(GL_TEXTURE_2D, textureId);
			glTexImage2D(GL_TEXTURE_2D, 0, format, picWidth, picWidth, 0, format, GL_UNSIGNED_BYTE, data);
